...
class Explainer:
    ...
    def explain(
        ...
        #     sub_label = self.target[graph_idx]
        #     neighbors = np.asarray(range(self.adj.shape[0]))
        # else:
        # print("node label: ", self.target[graph_idx][node_idx])
        node_idx_new, sub_adj, sub_data, sub_target, neighbors = self.extract_neighborhood(
            node_idx, graph_idx
        )
        print("neigh graph idx: ", node_idx, node_idx_new)
        # sub_target = np.expand_dims(sub_target, axis=0)
        #
        # sub_adj = np.expand_dims(sub_adj, axis=0)
        # sub_feat = np.expand_dims(sub_data, axis=0)
        #
        # adj   = torch.tensor(sub_adj, dtype=torch.float)
        # x     = torch.tensor(sub_feat, requires_grad=True, dtype=torch.float)
        # target = torch.tensor(sub_target, dtype=torch.long)
        adj = sub_adj
        x = sub_data
        target = sub_target

        # if self.graph_mode: # graph_mode 需要修改- Min
        #     pred_label = self.pred #np.argmax(self.pred[0][graph_idx], axis=0)
        #     print("Graph predicted label: ", pred_label)
        # else:
        #     pred_label = self.pred[:,:,neighbors,:] #np.argmax(self.pred[graph_idx][neighbors], axis=1)
        ...
        if True:
            ...
            for epoch in range(self.args.num_epochs):
                ...
                loss = explainer.loss(ypred,  node_idx_new, epoch)#pred_label,
                loss.backward()
                ...
                single_subgraph_label = sub_target.squeeze()
    ...
    def extract_neighborhood(self, node_idx, graph_idx=0):
        ...
        neighbors_adj_row = self.neighborhoods[node_idx, :] #self.neighborhoods[graph_idx][node_idx, :]
        # index of the query node in the new adj
        node_idx_new = sum(neighbors_adj_row[:node_idx])
        neighbors = np.nonzero(neighbors_adj_row)[0]

        for batch_index, batch_data in enumerate(self.dataload):
            encoder_inputs, decoder_inputs, labels = batch_data
            if batch_index ==0:
                sub_encoder_input = encoder_inputs[:, neighbors,:,:]
                sub_decoder_input = decoder_inputs[:,neighbors,:,:]
                sub_target = labels[:, neighbors,:,:]
            else:
                sub_encoder_input = torch.cat([sub_encoder_input, encoder_inputs[:, neighbors,:,:]],0)
                sub_decoder_input = torch.cat([sub_decoder_input,decoder_inputs[:,neighbors,:,:]],0)
                sub_target = torch.cat([sub_target,labels[:, neighbors,:,:]],0)

        sub_adj = self.adj[neighbors][:, neighbors]#self.adj[graph_idx][neighbors][:, neighbors]
        sub_data = torch.utils.data.TensorDataset(sub_encoder_input,sub_decoder_input,sub_target)
        # sub_feat = self.feat[graph_idx, neighbors] #self.feat[graph_idx, neighbors]
        # sub_label = self.label[graph_idx][neighbors] #self.label[graph_idx][neighbors]
        return node_idx_new, sub_adj, sub_data,sub_target,neighbors#node_idx_new, sub_adj, sub_feat, sub_label, neighbors
...
class ExplainModule(nn.Module):
    def __init__(
        ...
        self.feat_mask = self.construct_feat_mask(dataload[0][0].shape[-2], init_strategy="constant") #x.size(-1)
    ...
    def _masked_adj(self):
        ...
        return masked_adj #* self.diag_mask Min added
    ...
    def forward(self, node_idx, unconstrained=False, mask_features=True, marginalize=False):
        ...
        # ypred, adj_att = self.model(x, self.masked_adj) # outputs = net(encoder_inputs, decoder_inputs)
        # if self.graph_mode:
        ...
        for batch_index, batch_data in enumerate(self.dataload):
            ...
            # optimizer.zero_grad()
            outputs = self.model(encoder_inputs, decoder_inputs, self.masked_adj).squeeze()

            loss_pred = criterion(outputs[:, :, 0], labels[:, :, 0]) +  criterion(outputs[:, :, 1],
                                                                                      labels[:, :, 1])
            ...
            L = D - m_adj
            pred_label_t = outputs[:, :, 0].clone().detach()
            pred_label_c = outputs[:, :, 1].clone().detach()
            ...
            lap_loss = (self.coeffs["lap"]
                * (pred_label_t.transpose(0,1) @ L @ pred_label_t)
            )
            # lap_loss_list = [torch.sum(torch.diag(lap_loss))/self.masked_adj.numel() for i in range(lap_loss.shape[0])]
            # lap_loss_array = torch.stack(lap_loss_list)
            lap_loss = torch.sum(torch.diag(lap_loss))/self.masked_adj.numel()

            lap_loss_c = (self.coeffs["lap"]
                          * (pred_label_c.transpose(0, 1) @ L @ pred_label_c)
                          )
            # lap_loss_list_c = [torch.sum(torch.diag(lap_loss_c[i, :, :], 0)) / self.masked_adj.numel() for i in
            #                    range(lap_loss_c.shape[0])]
            # lap_loss_array_c = torch.stack(lap_loss_list_c)
            lap_loss_c =torch.sum(torch.diag(lap_loss_c))/self.masked_adj.numel()# torch.mean(lap_loss_array_c)

            loss1 = loss_pred +  lap_loss + lap_loss_c
            loss += loss1.item()
        ...
        return loss, self.masked_adj#res, adj_att
    ...
    def loss(self, pred, node_idx, epoch):#pred_label,
        """
        Args:
        ...
        print('pred_loss %.4f + size_loss %.3f +  mask_ent_loss%.3f'% (pred_loss , size_loss,  mask_ent_loss))
        if self.writer is not None:
