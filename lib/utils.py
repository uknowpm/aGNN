...
def tensor_norm_Adj(W):
    ...
    N = W.shape[0]
    W = W + torch.eye(N).to(W.device)  # 为邻接矩阵加上自连接
    D = torch.diag(1.0 / torch.sum(W, axis=1))
    # D = np.diag(1.0/np.sum(W, axis=1))
    norm_Adj_matrix = torch.mm(D,W)#np.dot(D, W)

    return norm_Adj_matrix


def compute_val_loss(net, val_loader, criterion, sw, epoch,adj):
    '''
    ...
    :return: val_loss
    '''
    # adj = torch.from_numpy(norm_Adj(adj)).type(torch.FloatTensor).to(DEVICE)
    adj = tensor_norm_Adj(adj)
    ...
    with torch.no_grad():
        ...
        for batch_index, batch_data in enumerate(val_loader):
            ...
            decoder_inputs = decoder_inputs.transpose(-1, -2) #decoder_inputs = decoder_inputs.unsqueeze(-1)  # (B, N, T, 1) ->(B, N, T, F)
            ...
            # encode
            encoder_output = net.encode(encoder_inputs,adj)
            ...
            for step in range(predict_length):
                ...
                predict_output = net.decode(decoder_inputs, encoder_output,adj)
...
def predict_and_save_results(net, data_loader, data_target_tensor, epoch, _max, _min, params_path, type,adj, index_adj,nrow,ncol):
    '''
    for transformerGCN
    :param net: nn.Module
    ...
    with torch.no_grad():
        ...
        fig, ax = plt.subplots(3, pred_wholemap.shape[2], figsize=(20, 3))
        for i in range(pred_wholemap.shape[2]):
            im1 = ax[0][i].imshow(pred_wholemap[ :,:, i])
            im2 = ax[1][i].imshow(tag_wholemap[ :,:, i])
            im3 = ax[2][i].imshow(
                pred_wholemap[ :,:, i]- tag_wholemap[ :,:, i])
...
def predict_results(net, data_loader, data_target_tensor, epoch, _max, _min, params_path, type):
    '''
    for transformerGCN
    :param net: nn.Module
    :param data_loader: torch.utils.data.utils.DataLoader
    ...
    with torch.no_grad():
        ...
        return prediction,data_target_tensor,mae
