...
def predict_and_save_results(net, data_loader, data_target_tensor, epoch, _max, _min, params_path, type):
    ...
    with torch.no_grad():
        ...
        b = np.concatenate((np.ones((1,prediction.shape[2]))*np.array(np.nan),prediction[66,:,:,0]),axis=0)
        b_t = np.concatenate((np.ones((1,data_target_tensor.shape[2]))*np.array(np.nan),data_target_tensor[66,:,:]),axis=0)
        from matplotlib import pyplot as plt
        fig, ax = plt.subplots(3, b.shape[1], figsize=(12, 3))
        for i in range(b.shape[1]):
            im1 = ax[0][i].imshow(b[ :, i].reshape((20, 10)))
            im2 = ax[1][i].imshow(b_t[ :, i].reshape((20, 10)))
            im3 = ax[2][i].imshow(
                b[ :, i].reshape((20, 10)) - b_t[ :, i].reshape((20, 10)))
        fig.colorbar(im1, ax=ax[0], shrink=0.85, pad=0.01)
        fig.colorbar(im2, ax=ax[1], shrink=0.85, pad=0.01)
        fig.colorbar(im3, ax=ax[2], shrink=0.85, pad=0.01)
        plt.show()
